...
export function useDashboardData(): DashboardData {
  ...
  useEffect(() => {
    console.log('[useDashboardData] 1️⃣ useEffect — orgLoading:', orgLoading, 'organizationId:', organizationId);
    ...
    const fetchData = async () => {
      setLoading(true);
      console.log('[useDashboardData] 2️⃣ Buscando dados para org:', organizationId);
      ...
      try {
        ...
        // Log any errors
        if (leadsRes.error) console.error('[useDashboardData] leads error:', leadsRes.error);
        if (salesRes.error) console.error('[useDashboardData] sales error:', salesRes.error);
        ...
        console.log('[useDashboardData] ✅ Dados:', { totalLeads, clients, totalRevenue, stages: stages.length, origins: origins.length });
