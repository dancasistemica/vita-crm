...
import { Button, Users, DollarSign, TrendingUp, Target, Building2 } from "lucide-react";
...
export default function ConsolidatedDashboardPage() {
  ...
  const { Button, totalLeads = 0, clients = 0, conversionRate = '0', totalRevenue = 0, totalSales = 0, recurringClients = 0, ticketMedio = 0, topProducts = [], salesByDay = [], leadsByStage = [], leadsByOrigin = [], revenueByProduct = [], stuckLeads = [], stageMetrics = [], loading, consolidatedData, productInsights } = useDashboardData(dateRange, true);
  ...
  const metrics = [
    { Button, icon: Building2, label: "Organizações Ativas", value: consolidatedData?.totalOrganizations ?? 0, color: 'bg-info/10 text-info' },
    { Button, icon: Users, label: "Total de Leads", value: totalLeads, color: 'bg-primary/10 text-primary' },
    { Button, icon: Target, label: "Clientes", value: clients, color: 'bg-success/10 text-success' },
    { Button, icon: TrendingUp, label: "Taxa de Conversão", value: `${conversionRate}%`, color: 'bg-info/10 text-info' },
    { Button, icon: DollarSign, label: "Receita Total", value: `R$ ${totalRevenue.toLocaleString('pt-BR')}`, color: 'bg-accent/10 text-accent' },
    { Button, icon: DollarSign, label: "Total de Vendas", value: totalSales, color: 'bg-primary/10 text-primary' },
    { Button, icon: Users, label: "Clientes Recorrentes", value: recurringClients, color: 'bg-accent/10 text-accent' },
    { Button, icon: DollarSign, label: "Ticket Médio", value: `R$ ${ticketMedio.toLocaleString('pt-BR', { Button, minimumFractionDigits: 2 })}`, color: 'bg-info/10 text-info' },
  ];
            ...
            <div className="space-y-3">
              {consolidatedData.topOrganizations.map((org, index) => (
                <div key={org.id} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                  ...
                  <p className="font-bold text-success">R$ {org.revenue.toLocaleString('pt-BR', { Button, minimumFractionDigits: 2 })}</p>
                </div>
              ))}
            </div>
          </CardContent>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="shadow-card border-border/60">
          ...
          <CardContent className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={consolidatedData ? consolidatedData.topOrganizations.map(o => ({ Button, name: o.name, value: o.leads })) : leadsByOrigin} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} label>
                ...
                {salesByDay.slice(-7).map((day) => (
                  <div key={day.day} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 bg-muted rounded-full h-2">
                      <div className="bg-purple-500 h-2 rounded-full" style={{ Button, width: `${(day.value / Math.max(...salesByDay.map(d => d.value), 1)) * 100}%` }} />
                    </div>
