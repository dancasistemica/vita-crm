...
export const AlertsList = ({ alerts, onResolve, onContact, loading }: AlertsListProps) => {
  if (loading) {
    return (
      <div className="space-y-4">
        {[1, 2, 3].map((i) => (
          <Card key={i} className="p-6 animate-pulse bg-neutral-50 h-32" />
        ))}
      </div>
    );
  }
  ...
  if (alerts.length === 0) {
    return (
      <Card className="p-8 text-center flex flex-col items-center justify-center border-dashed">
        ...
        <h3 className="text-lg font-medium text-neutral-900">Nenhum alerta de churn</h3>
  ...
  return (
    <div className="space-y-4">
      <Alert variant="info" className="bg-primary-50 border-primary-200 text-primary-800">
        <AlertCircle className="h-4 w-4" />
        <span>Foram detectados <strong>{alerts.length} clientes</strong> com 3 ou mais faltas reais consecutivas.</span>
      </Alert>

      {alerts.map((alert) => (
        <Card key={alert.id} className="p-6 overflow-hidden border-l-4 border-l-primary-500">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                <h3 className="text-lg font-bold text-neutral-900">{alert.client_name}</h3>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
                <div className="bg-neutral-50 p-2 rounded border border-neutral-100">
                  <span className="text-[10px] uppercase text-neutral-400 block font-semibold">Faltas Reais</span>
                  <span className="text-xl font-bold text-error-600">{alert.consecutive_real_absences}</span>
                  <span className="text-xs text-neutral-500 block">consecutivas</span>
                </div>
                <div className="bg-neutral-50 p-2 rounded border border-neutral-100">
                  <span className="text-[10px] uppercase text-neutral-400 block font-semibold">Ao Vivo</span>
                  <span className="text-xl font-bold text-primary-600">{alert.attended_live}</span>
                  <span className="text-xs text-neutral-500 block">presenças</span>
                </div>
                <div className="bg-neutral-50 p-2 rounded border border-neutral-100">
                  <span className="text-[10px] uppercase text-neutral-400 block font-semibold">Gravadas</span>
                  <span className="text-xl font-bold text-success-600">{alert.watched_recorded}</span>
                  <span className="text-xs text-neutral-500 block">assistidas</span>
                </div>
                <div className="bg-neutral-50 p-2 rounded border border-neutral-100">
                  <span className="text-[10px] uppercase text-neutral-400 block font-semibold">Última Falta</span>
