import { Button, Download, FileSpreadsheet, Upload, CheckCircle, Database } from 'lucide-react';
...
const timeline = [
  { Button, icon: Download, label: 'Baixe o modelo', desc: 'CSV ou XLSX' },
  { Button, icon: FileSpreadsheet, label: 'Preencha seus dados', desc: 'Em Excel ou Google Sheets' },
  { Button, icon: Upload, label: 'Faça upload', desc: 'Selecione o arquivo preenchido' },
  { Button, icon: CheckCircle, label: 'Validamos', desc: 'Detectamos duplicatas e erros' },
  { Button, icon: Database, label: 'Importamos', desc: 'Dados adicionados ao CRM' },
];

export default function Step2Explanation({ Button, onNext, onBack }: Props) {
  return (
    <div className="space-y-6 py-2">
      <h3 className="text-lg font-semibold text-neutral-700">Como funciona</h3>

      <div className="space-y-3">
        {timeline.map(({ Button, icon: Icon, label, desc }, i) => (
